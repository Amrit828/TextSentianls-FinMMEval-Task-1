...
from openai import OpenAI, BadRequestError, RateLimitError, APITimeoutError

import task1_faiss_rag_openai as base


# -----------------------------
# Groq routed ensemble v3
# -----------------------------
GROQ_API_KEY_ENV = "GROQ_API_KEY"
GROQ_BASE_URL = "https://api.groq.com/openai/v1"

ROUTER_MODEL = "qwen/qwen3-32b"
FACTUAL_MODEL = "qwen/qwen3-32b"
REASONING_MODEL = "openai/gpt-oss-20b"

INDEX_DIR = "task1_faiss_store_groq_v3"
RUN_MODE = "eval"
...
TOP_K_REASONING = 1
LIMIT = 50
PREDICTIONS_PATH = "task1_groq_routed_ensemble_v3_predictions.jsonl"
...
def groq_chat_create_with_retry(client: OpenAI, **kwargs):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            kwargs_with_timeout["timeout"] = REQUEST_TIMEOUT_SECONDS
            return client.chat.completions.create(**kwargs_with_timeout)
        ...
        "messages": [
            {
                "role": "system",
                "content": (
                    "You are a routing classifier for financial MCQ solving. "
...
def build_scoring_prompt(
    ...
    example: base.MCQExample,
    retrieved: list[tuple[base.MCQExample, float]],
) -> str:
    route = route_info["route"]
    if route == "reasoning_math":
        route_instruction = (
            "This is likely calculation-heavy or multi-step. "
            "Reason privately, use retrieval only if directly relevant, and score each option."
        )
    elif route == "exhibit_best_effort":
        route_instruction = (
            "This may reference an exhibit or passage. "
            "If some context seems missing, infer from the visible question and options anyway."
        )
    else:
        route_instruction = (
            "This is likely factual or conceptual. "
            "Use retrieved examples as soft reference patterns, not as direct evidence."
        )
    ...
    return "\n\n".join(
        [
            ...
            "scores must map each option letter to a non-negative number. best_answer must be one option letter.",
            route_instruction,
...
def score_with_model(
    ...
    request_kwargs = {
        ...
        "messages": [
            ...
            {
                "role": "user",
                "content": build_scoring_prompt(route_info, example, retrieved),
            },
        ],
    }
                ...
                "schema": {
                    ...
                    "properties": {
                        "scores": {
                            ...
                            "properties": {
                                label: {"type": "number"} for label in option_labels
                            },
                        },
                        "best_answer": {
                            "type": "string",
...
def ensemble_scores(
    ...
    merged = {}
    for label in option_labels:
        merged[label] = weight_a * score_a["scores"][label] + weight_b * score_b["scores"][label]
...
def retrieve_for_route(
    ...
    corpus_examples,
    excluded_target_ids,
) -> list[tuple[base.MCQExample, float]]:
    route = route_info["route"]
    top_k = TOP_K_REASONING if route == "reasoning_math" else TOP_K_FACTUAL
...
def run_eval() -> None:
    ...
    with open(PREDICTIONS_PATH, "w", encoding="utf-8") as f:
        for i, example in enumerate(target_examples, start=1):
            ...
            if i == 1 or i % 10 == 0:
                print(
                    f"[{i}/{len(target_examples)}] route={route_info['route']} "
